...
export function AtRiskView({
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-semibold text-foreground">{t("atRisk.title")}</h1>
      ...
      <details className="rounded-xl border border-border bg-surface-muted/50 px-4 py-3">
        <summary className="cursor-pointer text-sm font-medium text-foreground/70">{t("atRisk.criteriaTitle")}</summary>
        ...
        <p className="mt-2 text-xs text-foreground/45">{t("atRisk.criteriaHint")}</p>
      </details>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
        {students.length === 0 ? (
          <div className="flex flex-col items-center gap-2 px-5 py-16 text-center">
            ...
            <p className="text-sm text-foreground/50">{t("atRisk.empty")}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-muted/60 text-left text-xs font-medium uppercase tracking-wide text-foreground/50">
                  <th className="px-5 py-3">{t("atRisk.student")}</th>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {s.parentPhone && (
                          <WhatsAppLink
                            phone={s.parentPhone}
                            message={s.message}
                            title={t("atRisk.contactParent")}
                          />
                        )}
                        {scope === "school" && (
                          <Link
                            href={`/directeur/bulletins/${s.id}`}
                            title={t("atRisk.viewRecord")}
